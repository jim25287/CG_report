...
```{r, results='asis', fig.width = 4, fig.height= 8}}
...
```{r, results='asis', fig.width= 10, fig.height=20}
plot_grid(plotlist = myplots[90:99], ncol = 2, labels = paste0(LETTERS[8], seq(1,10)))
```

**Difference(%)-Blood**

```{r, results='asis', fig.width= 10, fig.height=20}
plot_grid(plotlist = myplots[100:109], ncol = 2, labels = paste0(LETTERS[9], seq(1,10)))
```
...
2.  Endpoint: 整體而言，各組間Inbody, Blood具顯著差異。(PBF, HOMA-ß, Lipid profile(TG, TC, LDL)無顯著差異)

3.  Diet Profile: 飲食遵從度, 具顯著差異;攝取總熱量無顯著差異。

4.  減重成效: 整體具有顯著差異，但HOMA-ß, Lipid profile(TG, TC, LDL),無顯著差異

5.  Question:

    1.  攝取總熱量無顯著差異，但BMR呈下降趨勢。

    2.  Endpoint體脂率沒差，但∆體脂率有差。

    3.  Lipid profile整體沒差異，但減重成效越好，HDL降幅越明顯。

    *(PBF= Body fat percentage; TG: Triglyceride; TC: Total cholesterol; )*
