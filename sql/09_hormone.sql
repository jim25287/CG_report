...
FROM CROSSTAB(
  ...
  SELECT client_id, choices.score, COUNT(choices.score)
  FROM surveys_with_answers
  INNER JOIN LP_scores_of_choices AS choices
  ON choices.id = surveys_with_answers.choice_id
  GROUP BY client_id, choices.score
  UNION
  SELECT surveys_with_answers.client_id, choices.score, COUNT(choices.score)
  FROM surveys_with_answers
  INNER JOIN tclea_scores_of_choices AS choices
  ON choices.id = surveys_with_answers.choice_id
  GROUP BY client_id, choices.score
  $$, $$VALUES('L'), ('P'), ('t'), ('c'), ('l'), ('e'), ('a')$$
) AS ct("client_id" INT, "hormone_L" TEXT, "hormone_P" TEXT, "hormone_t" INT, "hormone_c" TEXT, "hormone_l" TEXT, "hormone_e" INT, "hormone_a" INT)
